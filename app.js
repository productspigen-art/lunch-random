(() => {
  ...
  function flavorBlurbHTML(name){
    const n = (name||'').trim();
    const MAP = {
      '김치찌개':['보글보글 끓는 매콤 구수함.','밥 한 숟갈에 국물 촵— 캬!'],
      '된장찌개':['구수한 향이 코끝 찌릿.','두부와 채소가 사각— 건강한 한 술.'],
      '순두부찌개':['부들부들 순두부에 칼칼 국물.','한 숟갈에 몸이 사르르 풀린다.'],
      '부대찌개':['진득한 햄 풍미 + 얼큰함.','라면사리 넣고 후루룩 쭉!'],
      '감자탕':['뼈에 붙은 살 뜯는 재미.','시원한 국물에 들깨 고소함 가득.'],
      '해장국':['속이 스르르 풀리는 깊은 맛.','뜨끈한 한 그릇에 숙취 안녕!'],
      '육개장':['칼칼하고 개운한 고깃결.','파 향 솔솔, 땀 한 방울— 시원!'],
      '삼계탕':['보양 한 그릇의 담백함.','찹쌀과 인삼 향이 은은— 든든.'],
      '설렁탕':['사골의 담백한 깊이.','후추 톡— 소금 살짝, 깔끔한 마무리.'],
      '갈비탕':['맑지만 진한 국물.','갈빗대 쏙— 살코기 결결이 부드럽다.'],
      '라멘':['진득한 스프에 꼬들 면발.','차슈 한 입, 후루룩— 행복 업!'],
      '우동':['탱글한 면발과 깔끔 국물.','어묵 향이 은근— 자꾸만 후루룩.'],
      '냉면':['차가운 육수의 청량감.','겨자 톡— 식초 사르르, 시원함 폭발.'],
      '짜장면':['달큰 짜장에 면발 착—','단무지 아삭, 포크레인처럼 비벼먹자!'],
      '짬뽕':['불향 가득 얼큰함.','해물 한가득, 국물 한 모금에 캬~'],
      '초밥':['밥알 탱글 + 생선의 은은한 단맛.','와사비 톡— 간장 촉— 산뜻 깔끔.'],
      '사시미(회)':['차갑게 탱글한 식감.','기름기 살짝, 바다의 단맛이 스르륵.'],
      '돈카츠':['겉바속촉의 정석.','두툼한 고기에 바삭 소리— 소스 촉!'],
      '규동':['달큰 짭짤 소고기 + 양파.','뜨끈한 밥과 한 숟갈 비벼 쓱.'],
      '가츠동':['부드러운 카츠와 달걀의 만남.','양파 향이 스며든 촉촉한 한 끼.'],
      '사케동':['연어의 고소한 풍미가 가득.','유자/와사비 살짝— 상큼 담백.'],
      '텐동':['바삭 튀김과 단짠 소스.','밥과 함께 와앙— 만족도 만땅.'],
      '피자(페퍼로니)':['짭짤한 페퍼로니의 풍미.','치즈가 쭈욱— 한 입 행복.'],
      '파스타(까르보나라)':['크리미하고 부드러운 농도.','베이컨 짭짤함과 후추 향이 쏙.'],
      '파스타(알리오올리오)':['올리브오일의 담백함.','마늘 향 솔솔— 매력적인 심플함.'],
      '불고기':['달짝지근한 불향.','참기름 고소함과 쫄깃한 식감.'],
      '제육볶음':['매콤달콤 밥도둑의 정석.','김 한 장에 싸서— 촵!'],
      '쌀국수':['진한 육수에 허브의 산뜻함.','면발 후루룩— 개운하게 마무리.'],
      '분짜':['달짝지근 소스 + 숯불고기.','허브와 함께 상큼 바삭 조화.'],
      '반미':['바삭한 바게트에 촉촉 속재료.','고수 한 잎— 상큼함이 살아난다.'],
      '카레라이스':['향신료의 포근한 풍미.','버터의 고소함과 밥의 조화가 찰떡.'],
      '함박스테이크':['두툼한 패티에서 육즙 팡.','버터향과 소스가 입안을 감싼다.'],
    };
    const lines = MAP[n];
    if(lines){ return `<span class="blurb-title">맛 표현</span>${lines.map(t=>`<p>${t}</p>`).join('')}`; }
    // fallbacks by 패턴
    const is=(re)=>re.test(n);
    let a='', b='';
    if(is(/라멘|우동|짬뽕|칼국수|수제비|국수|국밥|탕|찌개/)){ a='뜨끈~한 국물에 후루룩 한 모금.'; b='속이 사르르— 오늘은 따뜻하게.'; }
    else if(is(/냉면|냉우동|소바|물냉|냉모밀/)){ a='차갑게 퍼지는 청량감!'; b='시원~하게 한 젓가락 쭉.'; }
    else if(is(/마라|불닭|매운|낙지|쭈꾸미|김치|칠리/)){ a='얼얼하게 쏴— 매콤함이 입안 가득!'; b='스트레스까지 싹—.'; }
    else if(is(/돈까스|카츠|가츠|튀김|치킨|양념치킨|후라이드|깐풍기|깐쇼/)){ a='겉바속촉의 바삭함!'; b='고소함이 팡팡— 자꾸만 손이 간다.'; }
    else if(is(/피자|리조또|스테이크|함박|감바스|파스타/)){ a='버터향+치즈의 진한 풍미.'; b='소스가 촉— 입에 감기는 만족감.'; }
    else if(is(/초밥|사시미|회덮밥|물회/)){ a='바다의 산뜻한 단맛.'; b='탱글한 식감이 스르륵— 깔끔.'; }
    else if(is(/카레|카레라이스|하이라이스/)){ a='향신료의 따뜻한 포근함.'; b='부드러운 소스에 밥이 술술.'; }
    else if(is(/덮밥|규동|가츠동|사케동|텐동|컵밥/)){ a='따끈한 밥 + 풍성한 토핑.'; b='비비는 순간 행복이 올라온다.'; }
    else if(is(/김밥|샌드위치|토스트|버거|반미|핫도그/)){ a='한 입에 쏙— 간편하지만 든든.'; b='와앙! 손맛이 즐거운 한 끼.'; }
    else if(is(/샐러드|샐러드파스타|분짜|쌀국수/)){ a='산뜻 아삭한 밸런스.'; b='상큼 담백, 가볍게 에너지 충전.'; }
    else { a='한입에 퍼지는 풍미!'; b='오늘도 든든하게 기분 업.'; }
    return `<span class="blurb-title">맛 표현</span><p>${a}</p><p>${b}</p>`;
  }
  ...
  function spinOnce(cond){
    ...
    setTimeout(()=>{
      ...
      if(els.resultBlurb) els.resultBlurb.innerHTML = flavorBlurbHTML(final.name);
