(() => {
  ...
  if(els.clearCatsBtn) els.clearCatsBtn.addEventListener('click', ()=>{ tempCond.cats = new Set(); renderCondSheet(); });
    ...
    els.shareBtn.addEventListener('click', async ()=>{
      const name = (state.lastPick || '').trim() || (els.result && (els.result.textContent||'').trim()) || '';
      const url = (typeof location !== 'undefined' && location.href) ? location.href : '';
      const text = name ? ¿À´Ã Á¡½É ÃßÃµ:  : '·ê·¿À» µ¹·Á ¿À´ÃÀÇ Á¡½ÉÀ» °ñ¶óº¸¼¼¿ä!';
      const payload = [text, url].filter(Boolean).join('\n');
      try{
        if(navigator.clipboard && navigator.clipboard.writeText){
          await navigator.clipboard.writeText(payload);
          els.shareBtn.textContent = 'º¹»çµÊ!';
          setTimeout(()=>{ els.shareBtn.textContent='°øÀ¯ÇÏ±â'; }, 1200);
        } else {
      ...
      if(navigator.share){
        try{ await navigator.share({ title: 'Á¡½É ÃßÃµ', text, url }); }catch{}
      } else {
        ...
        if(/Android/i.test(ua)){
          ...
          try{ window.open('mailto:?subject=' + encodeURIComponent('Á¡½É ÃßÃµ') + '&body=' + encodeURIComponent(payload), '_blank'); }catch{}
        }
      }
    });
        }
