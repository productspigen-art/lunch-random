(() => {
  ...
  const DEFAULT_ITEMS = [
    ...
    // ì¹˜í‚¨/?¨ìŠ¤??    {name:'?‘ë…ì¹˜í‚¨',cat:'fast'},{name:'ê°„ì¥ì¹˜í‚¨',cat:'fast'},{name:'?„ë¼?´ë“œì¹˜í‚¨',cat:'fast'},{name:'ë°˜ë°˜ì¹˜í‚¨',cat:'fast'},
    {name:'?µë‹­',cat:'fast'},{name:'ì¹˜í‚¨?¤í…Œ?´í¬',cat:'western'},
    // ë¹„ë¹”/??°¥/ë³¶ìŒë°?    {name:'ë¹„ë¹”ë°?,cat:'korean'},{name:'?Œì†¥ë¹„ë¹”ë°?,cat:'korean'},{name:'?œìœ¡??°¥',cat:'rice'},{name:'ë¶ˆê³ ê¸°ë®ë°?,cat:'rice'},
    ...
    // ?¼ì/?ŒìŠ¤?€/ë¦¬ì¡°??    {name:'?¼ì(?˜í¼ë¡œë‹ˆ)',cat:'fast'},{name:'?¼ì(ë§ˆë¥´ê²Œë¦¬?€)',cat:'fast'},{name:'ë¶ˆê³ ê¸°í”¼??,cat:'fast'},
    ...
    // ?„ì‹œ??    {name:'?€êµ?ˆ˜',cat:'vietnamese'},{name:'ë¶„ì§œ',cat:'vietnamese'},{name:'ë°˜ë?',cat:'vietnamese'},{name:'?Ÿí???,cat:'thai'},
  ...
  function getSharePayload(){
    ...
    const text = name ? `?¤ëŠ˜ ?ì‹¬ ì¶”ì²œ: ${name}` : 'ë£°ë ›???Œë ¤ ?¤ëŠ˜???ì‹¬??ê³¨ë¼ë³´ì„¸??';
    const title = '?ì‹¬ ì¶”ì²œ';
  ...
  if(els.shareBtn){
    els.shareBtn.addEventListener('click', async ()=>{
      ...
      const text = pick ? `?¤ëŠ˜ ?ì‹¬ ì¶”ì²œ: ${pick}` : 'ë£°ë ›???Œë ¤ ?¤ëŠ˜???ì‹¬??ê³¨ë¼ë³´ì„¸??';
      const url = (typeof location !== 'undefined' && location.href) ? location.href : '';
      // 1) Copy: current menu + page link
      const payload = [text, url].filter(Boolean).join('\n');
      try{
        if(navigator.clipboard && navigator.clipboard.writeText){
          await navigator.clipboard.writeText(payload);
          els.shareBtn.textContent = 'ë³µì‚¬??';
          setTimeout(()=>{ els.shareBtn.textContent='ê³µìœ ?˜ê¸°'; }, 1200);
        } else {
      ...
      // 2) Share sheet / intent / Web Share API
      if(navigator.share){
        try{ await navigator.share({ title: '?ì‹¬ ì¶”ì²œ', text, url }); }catch{}
      } else {
        const ua = navigator.userAgent || '';
        if(/Android/i.test(ua)){
          try{ location.href = 'sms:?body=' + encodeURIComponent(payload); }catch{}
        } else if(/iPhone|iPad|iPod/i.test(ua)){
          try{ location.href = 'sms:&body=' + encodeURIComponent(payload); }catch{}
        } else {
          try{ window.open('mailto:?subject=' + encodeURIComponent('?ì‹¬ ì¶”ì²œ') + '&body=' + encodeURIComponent(payload), '_blank'); }catch{}
        }
      }
    });
  }
    ...
    els.kakaoShareBtn.addEventListener('click', async ()=>{
      ...
      try{
        if(window.Kakao && window.Kakao.isInitialized && window.Kakao.isInitialized()){
          ...
          alert('ì¹´ì¹´?¤í†¡ ê³µìœ ë¥??¬ìš©?˜ë ¤ë©?Kakao ?????¤ì •???„ìš”?©ë‹ˆ?? meta[kakao-app-key] ?ëŠ” localStorage "lm_kakao_app_key"???¤ë? ?¤ì •??ì£¼ì„¸??');
        }
      }catch{
  ...
  if(els.copyShareBtn){
    els.copyShareBtn.addEventListener('click', async ()=>{
      ...
      try{
        if(navigator.clipboard && navigator.clipboard.writeText){
          await navigator.clipboard.writeText(payload);
          els.copyShareBtn.textContent = 'ë³µì‚¬??';
          setTimeout(()=>{ els.copyShareBtn.textContent='ë§í¬ ë³µì‚¬'; }, 1200);
        }else{
          const t=document.createElement('textarea'); t.value=payload; document.body.appendChild(t); t.select(); document.execCommand('copy'); document.body.removeChild(t);
          els.copyShareBtn.textContent = 'ë³µì‚¬??';
          setTimeout(()=>{ els.copyShareBtn.textContent='ë§í¬ ë³µì‚¬'; }, 1200);
